...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        #print "THIS IS THE CORNER PROBLEM."
        print "Starting state is at: ", "\n", startingGameState
        print "Corners are at: ", self.corners
    ...
    def getStartState(self):
        ...
        allCorners = []
        for corner in self.corners:
            allCorners.append(corner)	 # puting all corners into a list and eventually to the return
        #print "CORNERS ARE:", allCorners
        #print "STARTING POSITION IS " , self.startingPosition
        initialState = [0,0,0,0] 
        "this decides whether a corner is seen or not: if 0 not seen | if 1 seen"
        startState = (self.startingPosition, allCorners, initialState) 
        return startState
    ...
    def isGoalState(self, state):
        ...
        """
        #"""
        #print "len state", len(state[1])
        currentNode = state[0]
        allCorners = state[1]
        visitedFlag = state[2]
        
        sumFlag = 0;
        for i in range(0,len(visitedFlag)):
            sumFlag = sumFlag + visitedFlag[i]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #print "visitedFlag:" , visitedFlag
            
            "this loop checks the flag and makes a list full of corner which are not met"
            listOfCornersNotVisited = []
            for i in range(0,len(allCorners)):
                if visitedFlag[i] == 0 :
                    listOfCornersNotVisited.append(allCorners[i])
                
            #print "list of corner: ", listOfCornersNotVisited
            x,y = currentNode
            dx, dy = Actions.directionToVector(action)
            ...
            if hitsWall == False:
                newFlag = [0,0,0,0]
      	        "updating flag if there is any corner visited"
      	        for i in range(0,len(visitedFlag)):
      		    if nextNode == allCorners[i] :
      		        newFlag[i] = 1
      		    else :
      		        newFlag[i] = visitedFlag[i]

                #print "newFlag: ",newFlag
                first = (nextNode, allCorners, newFlag)
                result = [first, action, 1]
                #print "result: ", result
                successors.append(result)
            #"""
        
        self._expanded += 1
        #print "self._expanded: ", self._expanded
        return successors
...
def cornersHeuristic(state, problem):
    ...
    *** PASS: Heuristic resulted in expansion of 692 nodes --> I got 3/3
    ...
    #print "listOfCornersNotVisited:", listOfCornersNotVisited

    
    currentNodeComp = currentNode
    huristicReturn = 0
    
    while (len(listOfCornersNotVisited)!=0):
        
        #print "distance:", distance, "corner:", corner, "currentNodeComp:", currentNodeComp
        #currentNodeComp = corner
        
        "this is the list of all outputs of manhattan distance"
        outputManhattan = []
        "taking all manhattan distances into outputManhattan" 
        for corner in listOfCornersNotVisited:
            distanceLocal = util.manhattanDistance(currentNodeComp ,corner)
            #distanceLocal = mazeDistance(currentNodeComp ,corner, problem.startingGameState)
            b = [distanceLocal, corner]
            outputManhattan.append(b)
        
        minOutput = b
        
        "choosing minimum manhattan distance"
        for i in range (0, len(outputManhattan)):
            localMan = outputManhattan[i]
            if localMan[0] < minOutput[0]:
            #if localMan[0] > minOutput[0]:  # min/ max huristic. "<" is for minimum path and ">" is for maximum path
                minOutput[0] = localMan[0] 
                minOutput[1] = localMan[1]
        
        #print "distance:", minOutput[0], "corner:", minOutput[1], "currentNodeComp:", currentNodeComp
        #print "" 
        "updated huristic by adding the smallest distance to previous huristic and updating it"
        huristicReturn = huristicReturn + minOutput[0]
        
        "updated the current node to the current corner"
        currentNodeComp = minOutput[1]  
        
        "remove the current corner from the cornersNotVisited, this is much easier than just coparing the flags"
        listOfCornersNotVisited.remove(currentNodeComp)
        #break # here is the break that can be deactivated to activated

    return huristicReturn 
    #"""
...
def foodHeuristic(state, problem):
    """
    Your heuristic for the FoodSearchProblem goes here. for all three of them I used either
    manhattan distance or maze distance, in the beginning if the food list is empty the return 
    would be zero huristic which downgrades the problem to a A* without huristic.  
    ...
    *** 	thresholds: [15000, 12000, 9000, 7000]
    ...
    "if all the food is eaten then the food list should be empty. if empty return the huristic"
    if len( foodGridList) == 0:
        #print "foodGridList", foodGridList
        return huristicReturn
    
    else: 
        "this is essentially the same manhattanDistance mention in def 'cornersHeuristic(state, problem) which is modified"

        outputManhattan = []
        for foodNode in foodGridList:
            #distanceLocal = util.manhattanDistance(position ,foodNode)
            distanceLocal = mazeDistance(position ,foodNode, problem.startingGameState)
        
            b  = [distanceLocal, foodNode]
            outputManhattan.append(b)
        ...
        for i in range (0, len(outputManhattan)):
            ...
            if localMan[0] > minOutput[0]:
            #if localMan[0] > minOutput[0]:  # min/ max huristic. "<" is for minimum path and ">" is for maximum path
                minOutput[0] = localMan[0] # this is the distanceLocal
                minOutput[1] = localMan[1] # this is the foodNode
        
        huristicReturn = minOutput[0]
        return huristicReturn
        #return 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        firstElem = [(problem.getStartState(), "NA")]
        ...
        while True is True:
  	    ...
  	    for i in range(1,lenNode):
  	        "this loop always keeps the return direction updated to the latest path by taking the values form node and updaing"
  	        nodeXY = node[i]
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        #print "state: ", state
        #print "self.food[x][y]:  ", self.food[x][y] 
        #print "self.food: ", self.food
        #print " "

        #if state in self.food.asList():
        "if the food return is 1 it means the current node is food and goal achieved"
        x, y = state
        if self.food[x][y]==1:
           return True
        return False
