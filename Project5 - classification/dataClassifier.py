...
def enhancedPacmanFeatures(state, action):
    ...
    for this section I play with 2 variables:
    	- min distance to ghost 
    	- min distance to food
    
    SuicideAgent is An agent that only moves towards the closest ghost, 
    regardless of whether it is scared or not scared. Thus its feature
    should be maximized by inversing the value. minDdistanceGhost is 
    the most important parameter for suicide test
    
    ContestAgent is A staff agent from p2 that smartly avoids ghosts, 
    eats power capsules and food. Thus its feature should be maximized 
    by inversing the value. minDdistanceFood is the most important parameter 
    for contest test
    ...
    #pacman location, food location, ghost location
    pacmanNode = state.getPacmanPosition()
    foodNodes = state.getFood().asList()
    ghostNode = state.getGhostPositions()
    
    # minimum distance variable
    minDdistanceFood = 9999
    if len(foodNodes)!=0:
       for food in foodNodes:
          currentPosition = abs(pacmanNode[0]-food[0])+abs(pacmanNode[1]-food[1])
          if currentPosition<minDdistanceFood:
             minDdistanceFood = currentPosition
    #inverse distance as feature
    if minDdistanceFood!=0:
       features['foodDistance'] = 1.0/minDdistanceFood
       #features['foodDistance'] = minDdistanceFood
    else:
       features['foodDistance'] = 0

    
    # min distance of ghost
    minDdistanceGhost = 9999
    if len(ghostNode)!=0:
       for ghost in ghostNode:
          currentPosition = abs(pacmanNode[0]-ghost[0])+abs(pacmanNode[1]-ghost[1])
          if currentPosition<minDdistanceGhost:
             minDdistanceGhost = currentPosition
    # inverse of distance as feature
    if minDdistanceGhost!=0:
       features['GhostDistance'] = 1.0/minDdistanceGhost
       #features['GhostDistance'] = minDdistanceGhost
    else:
